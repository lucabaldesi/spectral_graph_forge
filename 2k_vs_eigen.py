...
def get_statistics1(G, H, duration):
    ...
    for i in range(sample_size):
        for j in range(i+1, sample_size):
            try:
                d = nx.algorithms.bidirectional_dijkstra(G, gsample[i], gsample[j])[0]
                dist1[d] = dist1.get(d, 0) + 1
            except:
                pass
            try:
                d = nx.algorithms.bidirectional_dijkstra(H, hsample[i], hsample[j])[0]
...
def csv_test():
    ...
    nets = ["fb_net/Michigan23.mat", "fb_net/MSU24.mat", "fb_net/UIllinios20.mat"] # ["fb_net/Reed98.mat", "fb_net/Caltech36.mat", "fb_net/Simmons81.mat"] # , "fb_net/Oberlin44.mat", "fb_net/Howard90.mat", "fb_net/Rice31.mat", "fb_net/Wake73.mat", "fb_net/UC64.mat", "fb_net/UCSC68.mat", "fb_net/Duke14.mat", "fb_net/UCF52.mat", "fb_net/Harvard1.mat" ]

    shots = []
#    for v in nm.arange(0.01, 1.01, 0.05):
#	for nodes in [100, 500, 1000]:
#		G = athina_net.athina_net(v, 0.5, 4, nodes)
#		while not nx.is_connected(G):
#		    print("Retrying with value: " + str(v))
#		    G = athina_net.athina_net(v)
#		# shots += [(str(v), G, "2k", 0)]
#		shots += [(str(v), G, "25k", 0)]
#		shots += [(str(v), G, "modularity", 0.9)]
    for net in nets:
	    G = read_mat_file(net)
            name = net.split("/")[1]
            name = name.split(".")[0]
